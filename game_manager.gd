extends Node

var quality_score = 0
var stars = 1


# Ingredient quantities
var ingredients = {
	"salt": 0,
	"sugar": 0,
	"curry_paste": 0,
	"coconut_milk": 0,
	"carrot": 0,
	"garlic": 0,
	"onion": 0
}


# Villager requests
var villager_requests = [
	"carrot",
	"onion"
]


# UI
@onready var star_label = $StarLabel


func _ready():

	update_ui()


# ADD INGREDIENTS

func add_ingredient(ingredient_name: String):

	ingredients[ingredient_name] += 1

	print(ingredient_name, " added")
	print(ingredients)

# BUTTON FUNCTIONS

func _on_salt_button_pressed():
	add_ingredient("salt")


func _on_sugar_button_pressed():
	add_ingredient("sugar")


func _on_curry_paste_button_pressed():
	add_ingredient("curry_paste")


func _on_coconut_milk_button_pressed():
	add_ingredient("coconut_milk")


func _on_carrot_button_pressed():
	add_ingredient("carrot")


func _on_garlic_button_pressed():
	add_ingredient("garlic")


func _on_onion_button_pressed():
	add_ingredient("onion")


# SERVE CURRY

func _on_serve_button_pressed():

	calculate_curry_quality()

# MAIN EVALUATION

func calculate_curry_quality():

	quality_score = 0


	# Salt
	if ingredients["salt"] == 1:
		quality_score += 1
	elif ingredients["salt"] > 1:
		quality_score -= 2
	else:
		quality_score -= 1


	# Sugar
	if ingredients["sugar"] == 1:
		quality_score += 1
	elif ingredients["sugar"] > 1:
		quality_score -= 2


	# Curry Paste
	if ingredients["curry_paste"] == 2:
		quality_score += 2
	elif ingredients["curry_paste"] > 2:
		quality_score -= 2
	elif ingredients["curry_paste"] == 0:
		quality_score -= 4


	# Coconut Milk
	if ingredients["coconut_milk"] == 1:
		quality_score += 1
	elif ingredients["coconut_milk"] > 1:
		quality_score -= 1


	# Garlic
	if ingredients["garlic"] == 1:
		quality_score += 1
	elif ingredients["garlic"] > 1:
		quality_score -= 1


	# Onion
	if ingredients["onion"] == 1:
		quality_score += 1
	else:
		quality_score -= 1


	# Villager requests
	for request in villager_requests:

		if ingredients[request] > 0:
			quality_score += 2
		else:
			quality_score -= 2


	calculate_star_rating()

	update_ui()

	print("Final Score: ", quality_score)

# STAR SYSTEM

func calculate_star_rating():

	if quality_score >= 8:
		stars = 5

	elif quality_score >= 6:
		stars = 4

	elif quality_score >= 4:
		stars = 3

	elif quality_score >= 2:
		stars = 2

	else:
		stars = 1

# UPDATE UI

func update_ui():

	star_label.text = "Stars: "

	for i in range(stars):

		star_label.text += "⭐"
